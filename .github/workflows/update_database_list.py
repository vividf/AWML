...
class AnnotationDatasetSearchClient:
    ...
    def search(self, keyword: str, project_ids: List[str]=["prd_jt", "x2_dev"], approved: bool=True):
        """
    ...
    def _fetch_all_annotation_datasets(self, payload: Dict[str, Any]):
        ...
        while True:
            ...
            response = self._session.post(url=self._base_url, headers=self._headers, params=params, data=json.dumps(payload))
            data = response.json()
...
def update_database_config(config_file_path, keyword, project_ids):
    ...
    found_datasets_ids = client.search(keyword=keyword, project_ids=project_ids)
    print(f"{len(found_datasets_ids)} found for keyword: {keyword} within projects: {project_ids}")
    ...
    for dataset_id in found_datasets_ids:
        if dataset_id not in config_content['train'] + config_content['val'] + config_content['test']:
            print(f"{dataset_id} seems to be a new ID, adding to the config file")
...
def main():
    ...
    parser.add_argument("--project_ids", type=str, nargs="*", default=["prd_jt", "x2_dev"], help="List of project IDs to filter by (optional)")
