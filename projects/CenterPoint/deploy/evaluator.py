...
class CenterPointEvaluator(BaseEvaluator):
    ...
    def __init__(
        ...
        Args:
            ...
            metrics_config: Optional configuration for the metrics adapter. Should be extracted
                           from model_cfg using extract_t4metric_v2_config() in the main script.
                           If None, default configuration will be used.

        Note:
            The metrics_config should be extracted from model_cfg using extract_t4metric_v2_config()
            in the main script. If metrics_config is None, default configuration will be used.
            Only T4MetricV2 is supported; T4Metric (v1) is not supported.
        """
        ...
        # Initialize metrics adapter for consistent evaluation with T4MetricV2
        if metrics_config is None:
            logger.info("Using default metrics configuration for deployment evaluation")
            metrics_config = Detection3DMetricsConfig(
                class_names=self.class_names,
                frame_id="base_link",
            )

        self.metrics_adapter = Detection3DMetricsAdapter(metrics_config)
    ...
    def _compute_metrics(
        ...
        This method uses Detection3DMetricsAdapter to ensure consistent metrics
        between deployment evaluation and training evaluation (T4MetricV2).
        """
        logger = logging.getLogger(__name__)

        # Count total predictions and ground truths for statistics
        ...
        # Use Detection3DMetricsAdapter for consistent metrics with T4MetricV2
        try:
            ...
            results = {
                ...
                # Primary metrics from autoware_perception_evaluation
            ...
            logger.info(f"   mAP: {results['mAP']:.4f}, mAPH: {results['mAPH']:.4f}")
