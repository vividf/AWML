...
# Spatial resolution (BEV input 1020×1020 from base grid_size):
#   stem_strides=(1,2,2) → 4× downsampling in stem → stem output 255×255.
#   no_pool_stages=(3,4,5) → stage3/4/5 do not add MaxPool → all stay at 255×255.
#   Stage3: 512ch @ 255×255  → upsample_stride=1 → 128ch @ 255×255
#   Stage4: 768ch @ 255×255  → upsample_stride=1 → 128ch @ 255×255
#   Stage5: 1024ch @ 255×255 → upsample_stride=1 → 128ch @ 255×255
#   Concat → 384ch @ 255×255 → head (out_size_factor=4 → 1020/4=255, matches).
model = dict(
    pts_backbone=dict(
        _delete_=True,
        type="BEVVoVNet",
        ...
        frozen_stages=-1,
        norm_eval=False,
        no_pool_stages=(3, 4, 5),  # stage3/4/5 all at 255×255 (no MaxPool)
    ),
    pts_neck=dict(
        type="SECONDFPN",
