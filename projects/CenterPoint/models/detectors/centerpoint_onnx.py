...
class CenterPointONNX(CenterPoint):
    ...
    def _get_random_inputs(self):
        ...
        """
        num_points = 200000
        points_tensor = torch.rand(num_points, self._point_channels, device=self._torch_device)

        point_cloud_range = getattr(self.pts_voxel_encoder, "point_cloud_range", None)
        if point_cloud_range is not None and len(point_cloud_range) >= 6:
            mins = torch.tensor(point_cloud_range[:3], device=self._torch_device, dtype=points_tensor.dtype)
            maxs = torch.tensor(point_cloud_range[3:6], device=self._torch_device, dtype=points_tensor.dtype)
            spatial = points_tensor[:, :3] * (maxs - mins) + mins
            points_tensor[:, :3] = spatial
    ...
    def _extract_random_features(self):
        ...
        print("voxel voxels dict: ", voxel_dict["voxels"].shape)
        print("voxel num_points dict: ", voxel_dict["num_points"].shape)
        print("voxel coors dict: ", voxel_dict["coors"].shape)
