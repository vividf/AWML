...
def get_rotation_bin(rot, dir_offset=0, num_bins=2, one_hot=True):
    ...
    if one_hot:
        bin_targets = torch.zeros(
            *list(bin_cls_targets.shape),
            num_bins,
            dtype=rot.dtype,
            device=bin_cls_targets.device)
        scatter_dim = len(bin_cls_targets.shape)
        bin_targets.scatter_(scatter_dim,
                             bin_cls_targets.unsqueeze(dim=-1).long(), 1.0)
...
def get_direction_bin(rot, dir_offset=0, one_hot=True):
    ...
    if one_hot:
        ...
        dir_targets.scatter_(scatter_dim,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self._class_wise_loss:
            print_log(
                "If you want to see a class-wise heatmap loss, use reduction='none' of 'loss_cls'."
            )
    ...
    def loss_by_feat(self, preds_dicts: Tuple[List[dict]],
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if self._class_wise_loss:
                ...
                loss_heatmap_cls = loss_heatmap_cls.sum(
                    (0, 2, 3)) / max(num_pos, 1)
                for cls_i, class_name in enumerate(class_names):
                    loss_dict[
                        f"task{task_id}.loss_heatmap_{class_name}"] = loss_heatmap_cls[
                            cls_i]
            else:
