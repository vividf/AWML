...
class FrustumRangePreprocessor(BaseDataPreprocessor):
    ...
    def __init__(
        ...
        super(FrustumRangePreprocessor, self).__init__(non_blocking=non_blocking)
        self.H = H
        self.W = W
    ...
    @torch.no_grad()
    def frustum_region_group(
        self,
        points_batch: List[Tensor],
        ...
        for i, res in enumerate(points_batch):
            ...
            if "pts_semantic_mask" in data_samples[i].gt_pts_seg:
                ...
                seg_label = (
                    torch.ones((self.H, self.W), dtype=torch.long, device=pts_semantic_mask.device) * self.ignore_index
                )
                ...
                min_value_row = torch.full(
                    (1, sorted_coors.size(1)),
                    torch.iinfo(sorted_coors.dtype).min,
                    dtype=sorted_coors.dtype,
                    device=sorted_coors.device,
                )
                ...
                voxel_sum = torch.zeros(
                    inverse_map.max() + 1, features.size(1), device=features.device, dtype=features.dtype
                )
                ...
                seg_label[res_voxel_coors[:, 1], res_voxel_coors[:, 2]] = voxel_semantic_mask
