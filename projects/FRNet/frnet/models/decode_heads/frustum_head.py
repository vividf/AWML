...
class FrustumHead(Base3DDecodeHead):
    ...
    def __init__(
        ...
        loss_ce: ConfigType = dict(
            type="mmdet.CrossEntropyLoss", use_sigmoid=False, class_weight=None, loss_weight=1.0
        ),
    ...
    def _stack_batch_gt(self, batch_data_samples: SampleList) -> Tensor:
        gt_semantic_segs = [data_sample.gt_pts_seg.semantic_seg for data_sample in batch_data_samples]
        return torch.stack(gt_semantic_segs, dim=0)

    def loss_by_feat(self, voxel_dict: dict, batch_data_samples: SampleList) -> dict:
        ...
        if self.loss_dice:
            loss["loss_dice"] = self.loss_dice(seg_logit, seg_label, ignore_index=self.ignore_index)
        if self.loss_lovasz:
    ...
    def predict(self, voxel_dict: dict, batch_input_metas: List[dict], test_cfg: ConfigType) -> List[Tensor]:
