    ...
    def __init__(
        ...
        if feat_compression is not None:
            self.compression_layers = nn.Sequential(
                nn.Linear(feat_channels[-1], feat_compression), nn.ReLU(inplace=True)
            )

    def forward(self, voxel_dict: dict) -> dict:
        features = voxel_dict["points"]
        inverse_map = voxel_dict["inverse_map"]
        voxel_dict["voxel_coors"] = voxel_dict["voxel_coors"][: inverse_map.max() + 1]
        ...
        if self._with_cluster_center:
            voxel_sum = torch.zeros(
                inverse_map.max() + 1, features.size(1), dtype=features.dtype, device=features.device
            )
            voxel_count = torch.zeros(
                inverse_map.max() + 1, features.size(1), dtype=features.dtype, device=features.device
            )
            ones_tensor = torch.ones_like(features)
            index_sum = torch.clamp(inverse_map.unsqueeze(-1).expand(-1, features.size(1)), 0, voxel_sum.size(0) - 1)
            index_count = torch.clamp(
                inverse_map.unsqueeze(-1).expand(-1, ones_tensor.size(1)), 0, voxel_count.size(0) - 1
            )
        ...
        voxel_feats = torch.full(
            (inverse_map.max() + 1, features.size(1)),
