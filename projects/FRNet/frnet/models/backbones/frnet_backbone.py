...
class FRNetBackbone(BaseModule):
    ...
    arch_settings = {18: (BasicBlock, (2, 2, 2, 2)), 34: (BasicBlock, (3, 4, 6, 3))}
    ...
    def _make_attention_layer(self, channels: int) -> nn.Module:
        return nn.Sequential(
            build_conv_layer(self.conv_cfg, channels, channels, kernel_size=3, padding=1, bias=False),
    ...
    def make_res_layer(
        ...
        if stride != 1 or inplanes != planes:
            downsample = nn.Sequential(
                build_conv_layer(conv_cfg, inplanes, planes, kernel_size=1, stride=stride, bias=False),
    ...
    def forward(self, voxel_dict: dict) -> dict:
        ...
        stride_voxel_coors, frustum_feats = self.point2frustum(point_feats, voxel_coors, inverse_map, stride=1)
        ...
        for i in range(len(outs)):
            outs[i] = F.interpolate(outs[i], size=target_size, mode="bilinear", align_corners=True)
    ...
    def frustum2pixel(self, frustum_features: Tensor, coors: Tensor, batch_size: int, stride: int = 1) -> Tensor:
        nx = self.nx // stride
        ny = self.ny // stride
        if self.deploy:
            batch_size = 1
    ...
    def point2frustum(
        self, point_features: Tensor, voxel_coors: Tensor, inverse_map: Tensor, stride: int = 1
    ) -> Tuple[Tensor, Tensor]:
        voxel_coors_strided = voxel_coors // stride

        frustum_features = torch.full(
            (inverse_map.max() + 1, point_features.size(1)),
