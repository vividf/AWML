...
def parse_args():
    ...
    parser.add_argument("--threshold", type=float, default=-999.9, help="Threshold for score.")
    parser.add_argument("--no-deploy", dest="deploy", action="store_false", help="Skip model deploy.")
...
def main():
    ...
    torch_model = TorchModel(deploy_cfg=deploy_cfg, model_cfg=model_cfg, checkpoint_path=args.checkpoint)
    ...
    trt_model = TrtModel(deploy_cfg=deploy_cfg, onnx_path=onnx_path, deploy=args.deploy, verbose=args.verbose)
