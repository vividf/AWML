...
def create_nuscenes_infos(root_path: str, info_prefix: str, version: str) -> None:
    ...
    if test:
        ...
        print(f"train scene: {len(train_scenes)}, " f"val scene: {len(val_scenes)}")
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(nusc, train_scenes, val_scenes, test)
    ...
    if test:
        ...
        print(f"train sample: {len(train_nusc_infos)}, " f"val sample: {len(val_nusc_infos)}")
...
def _fill_trainval_infos(
    ...
    for sample in mmengine.track_iter_progress(nusc.sample):
        ...
        if not test:
            pts_semantic_mask_path = osp.join(nusc.dataroot, nusc.get("lidarseg", lidar_token)["filename"])
...
if __name__ == "__main__":
    ...
    if args.version != "v1.0-mini":
        create_nuscenes_infos(args.root_path, args.extra_tag, f"{args.version}-trainval")
        create_nuscenes_infos(args.root_path, args.extra_tag, f"{args.version}-test")
    else:
