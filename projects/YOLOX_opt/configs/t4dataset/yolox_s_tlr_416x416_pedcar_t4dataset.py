_base_ = [
    ...
    "../../../../autoware_ml/configs/detection2d/dataset/t4dataset/tlr_finedetector.py"
]
...
test_pipeline = [
    dict(type='LoadImageFromFile', backend_args=backend_args),
    dict(type='LoadAnnotations', with_bbox=True),
    dict(type="RandomCropWithROI", crop_size=(1.0, 6.0)),
    dict(type='Resize', scale=img_scale, keep_ratio=True),
    dict(
        type='Pad',
        pad_to_square=True,
...
val_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=anno_file_root + "tlr_infos_test.json",    # Needs to be updated to tlr_infos_test.json once the dataset gets larger, and validation split is also added. 
...
test_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=anno_file_root + "tlr_infos_test.json",
        test_mode=True,
        pipeline=test_pipeline,
        backend_args=backend_args))

val_evaluator = dict(
    type='TLRFineDetectorEvaluator', metric='mAP', classes=_base_.classes)
