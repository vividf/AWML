...
class RandomCropWithROI(BaseTransform):
    ...
    def calculate_roi_crop(
            bbox: np.ndarray, img_h: int, img_w: int, crop_size: Tuple[float,
                                                                       float],
            hw_ratio: Tuple[float, float]) -> Tuple[int, int, int, int]:
        ...
        rand_box_center = roi_center + 0.5 * crop_size - np.random.rand(
            2) * crop_size
    ...
    def transform(self, results: Dict[str, any]) -> Optional[Dict[str, any]]:
        ...
        if "crop_coords" in results:
            ...
            crop_y1, crop_x1, crop_y2, crop_x2 = self.calculate_random_crop(
                img_h, img_w)
        ...
        if instances:
            ...
            for idx, instance in enumerate(instances):
                bbox = np.array(instance['bbox'])
                if (bbox[2] > bbox[0]) and (
                        bbox[2] - bbox[0]) > self.min_size and (
                            bbox[3]
                            > bbox[1]) and (bbox[3] - bbox[1]) > self.min_size:
                    valid_indices.append(idx)

            instances = [instances[idx] for idx in valid_indices]

            results["gt_bboxes"] = HorizontalBoxes(
                [gt_boxes[idx] for idx in valid_indices], dtype=torch.float32)
            results["gt_bboxes_labels"] = np.array(
                [gt_labels[idx] for idx in valid_indices], dtype=np.int64)
            results["gt_ignore_flags"] = np.array(
                [ignore_flag[idx] for idx in valid_indices], dtype=bool)
    ...
    def __repr__(self) -> str:
        ...
        repr_str += f"(crop_size={self.crop_size}, hw_ratio={self.hw_ratio}, process_target_only={self.process_target_only})"
        return repr_str
