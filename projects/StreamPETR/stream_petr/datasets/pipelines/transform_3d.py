...
class ResizeCropFlipRotImage:
    ...
    def __call__(self, results):
        ...
        for i in range(N):
            # Sample the augmentation independently per camera (resize jitter,
            # crop_x, flip). Independent per-view sampling is a strictly richer
            # augmentation than one draw shared by all cameras, and each view's
            # intrinsics are updated consistently below. At test time
            # _sample_augmentation is deterministic, so per-view calls are
            # identical to a single shared draw.
