...
class StreamPETRHead(AnchorFreeHead):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(StreamPETRHead, self).__init__(
            num_classes, in_channels, init_cfg=init_cfg, loss_bbox=loss_bbox, loss_cls=loss_cls, bbox_coder=bbox_coder
        )
    ...
    def loss_by_feat(
        self, cls_scores, bbox_preds, batch_gt_instances, batch_img_metas, batch_gt_instances_ignore=None
    ):
    ...
    def pre_update_memory(self, data):
        ...
        if torch.isnan(self.memory_egopose).any():
            print("Warning: memory_embedding is nan")
        if torch.isnan(self.memory_reference_point).any():
            print("Warning: memory_reference_point is nan")
        if torch.isnan(self.memory_timestamp).any():
            print("Warning: memory_timestamp is nan")
        if torch.isnan(self.memory_velo).any():
            print("Warning: memory_velo is nan")
        if torch.isnan(self.memory_embedding).any():
            print("Warning: memory_embedding is nan")
    ...
    def prepare_for_dn(self, batch_size, reference_points, targets, labels):
        if self.training and self.with_dn:
            ...
            boxes = torch.cat([torch.cat((t.gravity_center, t.tensor[:, 3:]), dim=1) for t in targets])
    ...
    def loss_single(
        ...
        loss_bbox = self.loss_bbox(
            bbox_preds[usable, :10],
            normalized_bbox_targets[usable, :10],
            bbox_weights[usable, :10],
    ...
    def get_bboxes(self, preds_dicts):
        ...
        for i in range(num_samples):
            ...
            ret_list.append([bboxes[mask], scores[mask], labels[mask]])
        return ret_list
