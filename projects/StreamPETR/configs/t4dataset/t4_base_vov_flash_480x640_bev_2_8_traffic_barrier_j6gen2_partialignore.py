...
optim_wrapper = dict(
    ...
    # The 2026-07-29 bf16 ablation (val 0.391 vs fp16 0.396) is INVALID as a
    # precision comparison: attention.py's unconditional `.half()` silently
    # downcast bf16 back to fp16 inside cross-attention, so that run never
    # changed the attention numerics. attention.py now honors the caller's
    # dtype; the honest bf16 recipe lives in the *_aligned_bf16.py variant.
    loss_scale="dynamic",
