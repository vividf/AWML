...
def basedetector__forward(
    ...
    if imgs is not None:
        ...
        img_aug_matrix = imgs.new_tensor(
            np.stack(data_samples[0].img_aug_matrix))
        img_aug_matrix_inverse = imgs.new_tensor(
            np.stack(
                [np.linalg.inv(x) for x in data_samples[0].img_aug_matrix]))
        ...
        batch_inputs_dict.update({
            ...
            'lidar2img':
            lidar2image.unsqueeze(dim=0),
            'cam2img':
            cam2image.unsqueeze(dim=0),
            'cam2lidar':
            camera2lidar.unsqueeze(dim=0),
            'img_aug_matrix':
            img_aug_matrix.unsqueeze(dim=0),
            'img_aug_matrix_inverse':
            img_aug_matrix_inverse.unsqueeze(dim=0),
            'lidar_aug_matrix':
            lidar_aug_matrix.unsqueeze(dim=0),
    ...
    one_hot = F.one_hot(
        outputs['query_labels'], num_classes=score.size(1)).permute(0, 2, 1)
    score = score * outputs['query_heatmap_score'] * one_hot
    score = score[0].max(dim=0)[0]

    bbox_pred = torch.cat([
        outputs['center'][0], outputs['height'][0], outputs['dim'][0],
        outputs['rot'][0], outputs['vel'][0]
    ],
                          dim=0)
