...
class SubclassHeadBBox(nn.Module):
    ...
    def head_forward(self, inputs, classes_eye):
        ...
        bev_pos = (
            self.bev_pos.to(lidar_feat.dtype)
            .repeat(batch_size, 1, 1)
            .to(lidar_feat.device)
        )
        ...
        top_proposals = heatmap.view(batch_size, -1).topk(
            k=self.num_proposals, dim=-1, largest=True
        )[1]
        ...
        #     0, 2, 1
        # ).half()
        self.one_hot = classes_eye.index_select(0, top_proposals_class.view(-1))[
            None
        ].permute(0, 2, 1)
...
def make_camera_onnx(args, model, cfg, save_root, points, img):
    ...
    depth = torch.zeros(
        len(points), img.shape[1], 1, img.shape[-2], img.shape[-1]
    ).cuda()
