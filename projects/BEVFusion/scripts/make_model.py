...
from .lean.quantize import (calibrate_model, disable_quantization, initialize,
                            print_quantizer_status, quantize_decoder,
                            quantize_encoders_camera_branch,
                            quantize_encoders_lidar_branch,
                            replace_to_quantization_module, set_quantizer_fast)
...
def parse_args():
    ...
    parser.add_argument(
        "--input_data", type=str, default="runs/example-data/example-data.pth")
    parser.add_argument(
        "--quantization", type=str, default="fp16", help="fp16, int8")
...
class SubclassHeadBBox(nn.Module):
    ...
    def __init__(self, parent):
        ...
        self.classes_eye = nn.Parameter(
            torch.eye(parent.heads.object.num_classes).float())
    ...
    def head_forward(self, inputs, classes_eye):
        ...
        lidar_feat_flatten = lidar_feat.view(batch_size,
                                             int(lidar_feat.shape[1]),
                                             -1)  # [BS, C, H*W]
        bev_pos = (
            self.bev_pos.to(lidar_feat.dtype).repeat(batch_size, 1,
        ...
        top_proposals = heatmap.view(batch_size, -1).topk(
            k=self.num_proposals, dim=-1, largest=True)[1]
        ...
        #     0, 2, 1
        # ).half()
        self.one_hot = classes_eye.index_select(
            0, top_proposals_class.view(-1))[None].permute(0, 2, 1)
...
def make_ptq_model(args, model, cfg, save_root):
    ...
    model.encoders.lidar.backbone = layer_fusion_bn(
    ...
    print("train nums:{} val nums:{}".format(
    ...
    model.module.encoders.lidar.backbone = fuse_relu_only(
...
def make_camera_onnx(args, model, cfg, save_root, points, img):
    ...
    depth = torch.zeros(
        len(points), img.shape[1], 1, img.shape[-2], img.shape[-1]).cuda()
    ...
    with torch.no_grad():
        ...
        print(
            f"The export is completed. ONNX save as {camera_vtransform_onnx}")
...
def make_lidar_onnx(args, model, cfg, save_root, points, img):
    ...
    lidar_backbone_onnx_path = os.path.join(save_root,
                                            lidar_backbone_onnx_name)
