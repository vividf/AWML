...
class SparseConvolution(SparseModule):
    ...
    def forward(self, input):
        ...
        if not self.subm:
            if self.transposed:
                out_spatial_shape = Fsp.get_deconv_output_size(
                    spatial_shape,
                    self.kernel_size,
                    self.stride,
                    self.padding,
                ...
                out_spatial_shape = Fsp.get_conv_output_size(
        ...
        if self.inverse:
            ...
            if self.indice_key is not None and data is not None:
                outids, _, indice_pairs, indice_pair_num, _, num_activate_out = data
            else:
                outids, indice_pairs, indice_pair_num, num_activate_out = Fsp.get_indice_pairs(
                    indices,
                    batch_size,
                    spatial_shape,
                    self.kernel_size,
        ...
        outids = outids[
            0:num_activate_out, :
        ]  # Note(kenzo): this is needed to add a slice in onnx!

        if device != indice_pairs.device:
            indice_pairs = indice_pairs.to(device)
        ...
        if self.fused_bn:
            ...
            if self.subm:
                out_features = Fsp.indice_subm_conv(
                    features, self.weight, indice_pairs, indice_pair_num, outids
                )
            else:
                if self.inverse:
