...
class SparseConvolution(SparseConvolutionBase):
    ...
    def _conv_forward(self,
        ...
        if self.indice_key is not None:
            ...
            if data is not None:
                msg = ('due to limitation of pytorch, '
                    ...
                    if self.indice_key is not None and data is not None:
                            ...
                            try:
                                res = \
                                    Fsp_custom.get_indice_pairs_implicit_gemm(
                                        ...
                                        algo, self.kernel_size, self.stride,
                                        self.padding, self.dilation,
                                        self.output_padding, self.subm,
                                        self.transposed,
                                        (not self.subm) or training,
                                        input.thrust_allocator, input._timer)
                            except Exception as e:
                                msg = '[Exception|implicit_gemm_pair]'
                                msg += (f'indices={indices.shape},'
                                        'bs={batch_size},'
                                        'ss={spatial_shape},')
                                msg += (f'algo={algo},'
                        ...
                        if self.indice_key is not None:
                            indice_data = ImplicitGemmIndiceData(
                                ...
                                dilation=self.dilation)
                            msg = (f'your indice key {self.indice_key} '
