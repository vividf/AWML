"""Parse-validation for every YOLOX deploy config.

Feeds each ``deployment/projects/yolox/config/deploy_config*.py`` through the typed parsers in
``deployment.config.schema`` to catch structural/rename errors WITHOUT a real checkpoint, CUDA, or a
full ``BaseDeploymentConfig`` (which validates checkpoint existence + CUDA). Requires CPU torch +
mmengine (the deployment runtime image); no GPU needed. Mirrors ``test_centerpoint_configs.py``.
"""
...
_CONFIG_DIR = Path(__file__).resolve().parents[1] / "projects" / "yolox" / "config"
...
def test_yolox_config_parses(config_path: Path) -> None:
    """Every YOLOX deploy config parses cleanly under the typed schema."""
    ...
    # YOLOX is a single whole-model component.
