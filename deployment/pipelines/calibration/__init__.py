...
PyTorch:
    >>> from deployment.pipelines.calibration import CalibrationPyTorchPipeline
    >>> pipeline = CalibrationPyTorchPipeline(model, device='cuda')
    >>> predictions, latency = pipeline.infer(input_tensor)

ONNX:
    >>> from deployment.pipelines.calibration import CalibrationONNXPipeline
    >>> pipeline = CalibrationONNXPipeline(onnx_path='model.onnx', device='cuda')
    >>> predictions, latency = pipeline.infer(input_tensor)

TensorRT:
    >>> from deployment.pipelines.calibration import CalibrationTensorRTPipeline
    >>> pipeline = CalibrationTensorRTPipeline(engine_path='model.engine', device='cuda')
    >>> predictions, latency = pipeline.infer(input_tensor)
"""

from deployment.pipelines.calibration.calibration_onnx import CalibrationONNXPipeline
from deployment.pipelines.calibration.calibration_pipeline import CalibrationDeploymentPipeline
from deployment.pipelines.calibration.calibration_pytorch import CalibrationPyTorchPipeline
from deployment.pipelines.calibration.calibration_tensorrt import CalibrationTensorRTPipeline
