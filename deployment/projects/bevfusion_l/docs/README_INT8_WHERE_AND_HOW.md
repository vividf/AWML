...
- **量化 + `implicit_gemm`**：`deployment/projects/bevfusion_l/cpp/int8_plugin/implicit_gemm_int8_plugin.cpp` 的 `enqueue`  
...
- **PyTorch 稀疏塔 fake quant**：`deployment/projects/bevfusion_l/quantization/spconv_int8.py`（`TensorQuantizer`，histogram → `_amax`）
...
| 說明 | 路徑（相對各 repo 根目錄） |
|------|---------------------------|
| AWML INT8 TRT plugin | `AWML/deployment/projects/bevfusion_l/cpp/int8_plugin/implicit_gemm_int8_plugin.cpp` |
