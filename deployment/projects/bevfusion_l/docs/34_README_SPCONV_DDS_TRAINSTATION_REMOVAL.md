...
>
> **Note (framework port).** This document was written against the pre-refactor deployment
> framework and ported into `deployment/projects/bevfusion_l/` afterwards. Current file map:
> graph surgery `export/onnx_remove_trainstation_dds.py`; naming contract / stage geometry /
> Python precompute `io/sparse_rulebook_inputs.py`; flag plumbing
> `config/bevfusion_deployment_config.py` + `config/component_layout.py`
> (`add_rulebook_input_profiles`); eval runtime `inference/tensorrt_inference_pipeline.py`;
> deploy cfg `config/deploy_config_split_fp16_remove_trainstation.py`. The `_ts_tmp/` scripts
> referenced in §8 were throwaway validation harnesses and were intentionally not ported —
> their durable pieces live in `deployment/tests/test_sparse_rulebook_inputs.py`.
...
(`io/sparse_rulebook_inputs.py` for AWML eval; `preprocess/sparse_rulebook_precompute.cu`
...
`io/sparse_rulebook_inputs.py`；autoware_bevfusion 在
...
`AWML/deployment/projects/bevfusion_l/export/onnx_remove_trainstation_dds.py`
...
> box (see §1.5). The name is produced by `onnx_remove_trainstation_dds.rulebook_input_name()` and
> consumed, by exact match, in three places that must stay in sync:
> 1. **Export** — `export/onnx_remove_trainstation_dds.py` (transform + consumer-edge rewrite).
> 2. **AWML eval runtime** — `io/sparse_rulebook_inputs.py` (`parse_rulebook_input_name`,
...
> `config/deploy_config_split_fp16_remove_trainstation.py`. (Earlier revisions kept the raw
...
- Export/build CLI:
  ...
  deploy cfg: `deployment/projects/bevfusion_l/config/deploy_config_split_fp16_remove_trainstation.py`.
...
- `export/component_builder.py`: registers `remove_trainstation_dds` as a sparse-component
  `post_transforms` entry when `deploy_cfg.spconv_remove_trainstation` is set (independent of the
  ReLU-fuse flag; composes cleanly with it — DDS removal runs first).
- `deploy_config_split_fp16_remove_trainstation.py`: `spconv_remove_trainstation = True`. The 16
  rulebook TensorRT-profile entries are *derived* from the `coors` profile by
  `config/component_layout.py::add_rulebook_input_profiles` (N bounded by the coors voxel-count
  envelope; KV=27 for l1–l3, 3 for conv_out), so deploy configs never restate them.
- Ran the official CLI end-to-end
  (`python -m deployment.cli.main bevfusion_l <trainstation cfg> <model cfg>`): log shows
...
The deployment's own Python TensorRT pipeline (`inference/tensorrt_inference_pipeline.py`) also needs
to supply the 16 rulebook inputs (eval failed with "Address is not set for input …
GetIndicePairsImplicitGemm_output_0" until wired). Added:
- `io/sparse_rulebook_inputs.py`: `compute_rulebook_inputs(coors_zyx, rulebook_names, sparse_shape)` —
  cascades `GetIndicePairsImplicitGemm` over the 4 down-sample stages (the validated Slice-1d logic;
  spatial shapes derived from the model's own `sparse_shape`), returns `{input_name: int32 np.ndarray}`.
  `split_rulebook_inputs()` gates it (no-op for baseline engines).
- `inference/tensorrt_inference_pipeline.py::_trt_infer_voxel_inputs`: when the engine exposes
  rulebook inputs, precompute from the same `coors` and add to the bind map before `enqueueV3`
  (covers both the split sparse engine and the merged full-graph engine).
...
2. **Merged-engine tensor-name prefix (engine builds, but profiles set on the wrong tensors).**
   ...
   trainStation rulebook inputs (added later by `onnx_remove_trainstation_dds`) kept the prefix and
   ...
   **Fix (export side, keeps the runtime clean):** in `export/transforms.py::merge_split_sparse_dense_onnx`, after
   ...
   `sparse_rulebook_inputs.split_rulebook_inputs` / `compute_rulebook_inputs` match prefix-agnostically.)
   *Requires re-exporting the ONNX with the fixed pipeline and rebuilding the engine.*
   ...
   > parses `rulebook/<tag>/<slot>` (`parse_rulebook_input_name`) and the autoware runtime uses
...
- **1b** Wire `remove_trainstation_dds` into `export/component_builder.py` as a sparse-ONNX post-process
