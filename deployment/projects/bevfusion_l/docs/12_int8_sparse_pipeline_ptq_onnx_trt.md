...
```mermaid
flowchart LR
  subgraph PTQ["PTQ（bevfusion/quantization/quantize.py）"]
  ...
  subgraph SparseInt8["sparse INT8（可選）"]
...
## 2. 階段一：PTQ（`deployment/quantization/bevfusion/quantization/quantize.py ptq`）
...
## 3. 階段二：Deploy 載入（`deployment/projects/bevfusion_l/io/model_loader.py`）
...
## 4. 階段三：匯出 ONNX（`deployment/projects/bevfusion_l/export/onnx_export_pipeline.py`）
...
| 層級 | 沒有 sparse INT8 時 | sparse INT8（`ImplicitGemmInt8` plugin）時 |
|------|----------------|----------------------------------------|
...
也就是說：**真正在 TRT 裡跑 INT8 GEMM kernel 的是 sparse INT8**；純 FP16 ONNX + 預設 plugin 路線是 **FP16 sparse conv**，與 PyTorch 端 INT8 校準是 **分離的兩套語意**（見 `10_int8_trt_gap_analysis.md`）。
...
- **作為**：把匹配的 `autoware::ImplicitGemm` 改成 **`autoware::ImplicitGemmInt8`**，並寫入 `input_scale` / `output_scale` / `channel_scale` / `bias_scaled`（公式見 `11_int8_autoware_plugin.md`）。  
...
# sparse INT8：ONNX 後處理（路徑依實際 work_dir）
...
| `11_int8_autoware_plugin.md` | sparse INT8 plugin、transform、Q/DQ 與 shadow 細節 |
