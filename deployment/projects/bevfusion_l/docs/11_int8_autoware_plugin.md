# sparse INT8: INT8 ImplicitGemm Plugin for Autoware TensorRT

## Overview

**sparse INT8** modifies the Autoware TensorRT `ImplicitGemm` plugin to run INT8
...
sparse INT8 keeps the entire stack open-source and maintainable.

### Path A vs sparse INT8 Comparison

| Aspect                | Path A (libspconv)                     | sparse INT8 (cumm INT8 Plugin)             |
|-----------------------|----------------------------------------|---------------------------------------|
...
```
deployment/projects/bevfusion_l/cpp/int8_plugin/
...
deployment/projects/bevfusion_l/export/
└── sparse_int8_onnx_transform.py            # ONNX post-processor

deployment/projects/bevfusion_l/benchmark/
...
```bash
cd deployment/projects/bevfusion_l/cpp/int8_plugin
...
以下為 sparse INT8 實作過程中 **逐步** 記下的修正、除錯方式與目前進度。**上文各節維持初稿，不作改寫**；若與本節有出入，以本節實務紀錄為準。

### 模組說明：`deployment.projects.bevfusion_l.export.sparse_int8_onnx_transform`

此模組是 **sparse INT8 的 ONNX 後處理器**（CLI：`python -m deployment.projects.bevfusion_l.export.sparse_int8_onnx_transform`）。標準 BEVFusion／Autoware 稀疏段匯出後，圖裡是 **`autoware::ImplicitGemm`**（5 個輸入：feature、weight、pair 相關等），數值仍是 **FP16 權重／特徵**；TRT 上的 FP16 plugin 不會自動帶入 PTQ 的 INT8 scale。本腳本把每顆可對齊校準的 `ImplicitGemm` **改成** **`autoware::ImplicitGemmInt8`**（7 個輸入），並寫入與 PTQ 一致的 scale／bias，讓後續載入 **`ImplicitGemmInt8` C++ plugin** 時能跑內部 INT8 kernel。
...
**輸出**：仍為 FP16 I／O 的稀疏 ONNX，但稀疏卷積節點變為 **`ImplicitGemmInt8`**，供 TensorRT 在載入 `libimplicit_gemm_int8_plugin.so` 時對應到 sparse INT8 plugin。無法對齊校準的節點（例如未量化的 `conv_out`）會 **維持原 `ImplicitGemm`**，並印 `[warn] Could not match …`。

### 紀錄 1：`ImplicitGemmInt8` C++ plugin 與 CMake

- 新增目錄 `deployment/projects/bevfusion_l/cpp/int8_plugin/`（見上文 **File Structure**）：`enqueue` 內 FP16→INT8 量化、`ConvGemmOps::implicit_gemm`、FP16 輸出。
...
| 改過 transform 但行為不變 | 刪除 `deployment/projects/bevfusion_l/export/__pycache__` 後重跑 |
...
**一句話**：「觸發 shadow」= **匯出時短暫換成「沒有 TensorQuantizer 的雙胞胎 FP32 稀疏塔」來 trace**，讓 ONNX 裡不要出現 Q/DQ；INT8 語意仍由後續 sparse INT8（`_amax` → `ImplicitGemmInt8`）或 PyTorch 推理路徑負責，與這顆乾淨 ONNX 相容。
...
- Path A 產物（與本 plugin 路線獨立）：`experimental/libspconv_onnx_exporter.py`、`experimental/export_sparse_encoder_int8.py`、`experimental/cpp/libspconv_trt_bridge.cpp` 等。
...
- **根因**：sparse INT8 為每層新增 `channel_scale` / `bias_scaled` **initializer**；除 `graph.initializer` 外需補 **`graph.input`（ValueInfo）**（見 **11** 紀錄 4）。
