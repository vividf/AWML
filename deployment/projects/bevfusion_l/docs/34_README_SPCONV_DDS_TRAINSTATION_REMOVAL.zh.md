...
>
> **框架移植说明。** 本文写于旧部署框架时期，之后移植到 `deployment/projects/bevfusion_l/`。
> 当前文件对照：图手术 `export/onnx_remove_trainstation_dds.py`；命名契约 / 阶段几何 / Python
> 预计算 `io/sparse_rulebook_inputs.py`；旗标接线 `config/bevfusion_deployment_config.py` +
> `config/component_layout.py`（`add_rulebook_input_profiles`，16 个 rulebook TRT profile 由
> `coors` profile 推导，deploy config 不再手写）；评估运行时
> `inference/tensorrt_inference_pipeline.py`；deploy cfg
> `config/deploy_config_split_fp16_remove_trainstation.py`。文中提到的 `_ts_tmp/` 脚本是一次性
> 验证工具，未随移植保留 — 可持久的部分改写为 `deployment/tests/test_sparse_rulebook_inputs.py`。
...
`AWML/deployment/projects/bevfusion_l/export/onnx_remove_trainstation_dds.py`（`remove_trainstation_dds`）。删除 4 个下采样 `GetIndicePairsImplicitGemm` 节点，并把它们被消费的输出（`out[0..3]`；`out[4]` num_act_out 无消费者，丢弃）提升为图输入，每阶段共享一个符号维度。
...
- `export/component_builder.py (sparse `post_transforms`)`：当设置 `deploy_cfg.spconv_remove_trainstation` 时，对稀疏 ONNX 运行 `remove_trainstation_dds`（独立于 ReLU-fuse 标志；可与之干净组合）。
- `deploy_config_split_fp16_remove_trainstation.py`：`spconv_remove_trainstation = True` + 以编程方式把 16 个 rulebook 输入注入 `components.bevfusion_sparse.tensorrt_profile`（N∈[1,256000]；KV=27 用于 l1–l3，3 用于 conv_out）。
- 端到端运行官方 CLI（`python -m deployment.cli.main bevfusion_l <trainstation cfg> <model cfg>`）：日志显示 "trainStation/DDS removal done (removed 4 … added 16 rulebook graph inputs)"，之后两个引擎都构建。**对 CLI 产出的 `bevfusion_sparse.engine` 做 engine-inspector：127 层，0 个 trainStation 层。**（与 ImplicitGemm ReLU 融合共存，13 个 relu。）
...
部署自己的 Python TensorRT 流水线（`inference/tensorrt_inference_pipeline.py`）也需要提供这 16 个 rulebook 输入（在接通之前，评估会以 "Address is not set for input … GetIndicePairsImplicitGemm_output_0" 失败）。新增：
- `io/sparse_rulebook_inputs.py`：`compute_rulebook_inputs(coors_zyx, input_names)`——在 4 个下采样阶段级联 `GetIndicePairsImplicitGemm`（已验证的切片 1d 逻辑），返回 `{input_name: int32 np.ndarray}`。`split_rulebook_inputs()` 作为门控（基线下为 no-op）。
- `inference/tensorrt_inference_pipeline.py::_trt_infer_voxel_inputs`：当稀疏引擎暴露 rulebook 输入时，从同一个 `coors` 预计算，并在 `enqueueV3` 之前加入绑定 map。
...
2. **合并引擎的张量名前缀（引擎能构建，但 profile 设到了错误的张量上）。**
   ...
   （`voxels`/`coors`/`num_points_per_voxel`）改回原名——所以后加（由 `onnx_remove_trainstation_dds` 添加）的
   ...
   **修正（放在导出端，保持运行时干净）：** 在 `export/transforms.py::merge_split_sparse_dense_onnx` 中，在声明输入/输出改名之后，
   ...
   （AWML 评估不受影响：`sparse_rulebook_inputs.split_rulebook_inputs` / `compute_rulebook_inputs` 按
...
- **1b** 把 `remove_trainstation_dds` 接入 `export/component_builder.py`，作为由 deploy-cfg 标志（如 `spconv_remove_trainstation=True`）门控的稀疏 ONNX 后处理；把 16 个输入加入 TensorRT 优化 profile（deploy cfg `tensorrt_profile`）。
