...
python -m deployment.cli.main bevfusion \
  deployment/projects/bevfusion_l/config/deploy_config_split.py \
...
| 檔案 | 用途 |
|------|------|
| `deployment/projects/bevfusion_l/export/onnx_export_pipeline.py` | 單檔或 **拆段** ONNX 匯出 |
| `deployment/projects/bevfusion_l/config/deploy_config_split.py` | **路線 1** 雙 component（FP） |
| `deployment/projects/bevfusion_l/config/deploy_config_split_int8.py` | **路線 1** + PTQ checkpoint / spconv INT8 |
| `deployment/projects/bevfusion_l/quantization/spconv_quantized_add_patch.py` | **不改 spconv 安裝檔**的 runtime patch |
| `deployment/projects/bevfusion_l/quantization/spconv_int8.py` | prepare_fx / convert_fx 流程 |
| `deployment/projects/bevfusion_l/io/model_loader.py` | PTQ 載入、結構對齊 |
