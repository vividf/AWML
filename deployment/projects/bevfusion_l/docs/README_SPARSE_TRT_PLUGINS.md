# Sparse Convolution TensorRT Plugins（Autoware / AWML sparse INT8）
...
適用對象：已熟悉 BEVFusion 稀疏塔匯出（`autoware::ImplicitGemm*`、`GetIndicePairsImplicitGemm`）與 sparse INT8 ONNX 後處理（`sparse_int8_onnx_transform.py`）的開發者。
...
### 3.3 `ImplicitGemmInt8`（AWML `libimplicit_gemm_int8_plugin.so`，sparse INT8）
...
| **輸入（7）** | 前 5 個與 `ImplicitGemm` 相同；另加 **`channel_scale`**、**`bias_scaled`**（FP32 向量，長度 `C_out`），語意與 sparse INT8 ONNX 變換腳本寫入的 PTQ 公式一致。 |
...
**程式入口**：`AWML/deployment/projects/bevfusion_l/cpp/int8_plugin/implicit_gemm_int8_plugin.cpp`
...
AWML 專案中 **`plugin_registration.cpp`** 僅 `#include` creator 標頭，目的是 **強制連結單元**執行 `REGISTER_TENSORRT_PLUGIN` 的靜態註冊（見 `deployment/projects/bevfusion_l/cpp/int8_plugin/plugin_registration.cpp`）。
...
1. **與 ONNX 一致**  
   ...
   - Node **attribute 名稱**與 Creator 里解析的 **字串完全一致**（例如 sparse INT8 使用 `input_scale`、`output_scale`）。
...
| 內容 | 路徑（依你工作區） |
|------|------------------|
| INT8 ImplicitGemm + Creator + 註冊 | `AWML/deployment/projects/bevfusion_l/cpp/int8_plugin/` |
| FP16 ImplicitGemm + GetIndicePairs | `autoware.universe/perception/autoware_tensorrt_plugins/` |
| ONNX 替換為 `ImplicitGemmInt8`、寫入 scale | `AWML/deployment/projects/bevfusion_l/export/sparse_int8_onnx_transform.py` |
| sparse INT8 公式與插件欄位 | `AWML/deployment/projects/bevfusion_l/docs/11_int8_autoware_plugin.md` |
...
若 **同一套 PTQ** 在 PyTorch 正常、TRT 僅在 **FP16 `ImplicitGemm` → `ImplicitGemmInt8` 邊界**異常，多半是 **兩顆 plugin 與 TRT 構圖互動**問題，而非單一公式；需結合 **層輸出對照**與 **ONNX 結構**排查（見專案內其他 sparse INT8 討論文檔）。
