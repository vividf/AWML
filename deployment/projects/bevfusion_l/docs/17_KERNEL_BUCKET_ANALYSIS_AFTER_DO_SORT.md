...
| 工具 | 你在看什麼 | 優點 | 限制 |
|---|---|---|---|
| `deployment/projects/bevfusion_l/benchmark/profile_sparse_encoder.py` | TensorRT `IProfiler` per-layer + CUDA-event 總時間 | 與部署路徑一致；輸出 bucket（`pair_gen` / `implicit_gemm_int8` / …） | 看不到 **具體 CUDA kernel 名稱** |
...
| `quant_act` | `quantize_features_kernel` | 每層 activation 的 FP16→INT8（sparse INT8 典型成本） |
...
在 `deployment/projects/bevfusion_l/config/deploy_config_split_int8.py` 裡設定：
