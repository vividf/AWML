...
### 2.1 PTQ 階段（`bevfusion/quantization/quantize.py`）
...
3. **AWML 補丁**：**`deployment/projects/bevfusion_l/quantization/spconv_quantized_add_patch.py`**  
...
**作法**：**`runner.py`**、**`bevfusion/quantization/quantize.py`**、**`spconv_int8.py`** 整併為單一清晰流程：**`apply_spconv_int8_quantization` → `calibrate_spconv_model` → `convert_spconv_int8`**；若 encoder 已是 FX 結果則 **跳過重複轉換**（依實作版本而定，詳見程式註解）。
...
| PTQ 入口 | `deployment/quantization/bevfusion/quantization/quantize.py` | PTQ 與 spconv INT8 整合 |
