...
| 入口 | 函式 | 檔案 |
|------|------|------|
| PTQ / deploy 載入 | `_fuse_spconv_bn(model)` | `deployment/projects/bevfusion_l/io/model_loader.py` |
| 實作 | `_fuse_spconv_bn_in_encoder` | `deployment/projects/bevfusion_l/quantization/spconv_int8.py` |
| spconv 工具 | `fuse_spconv_bn_eval` | `spconv/pytorch/quantization/utils.py` |
| FP32 shadow 重建（若使用） | 同上 | `deployment/projects/bevfusion_l/export/sparse_encoder_float_shadow.py` |
...
| 層級 | 倉庫 / 路徑 | 職責 | 若缺失的症狀 |
|------|-------------|------|----------------|
| **① BN fold** | `deployment/projects/bevfusion_l/quantization/spconv_int8.py` → `_fuse_spconv_bn_in_encoder`<br>`deployment/projects/bevfusion_l/io/model_loader.py` → `_fuse_spconv_bn`<br>`spconv/pytorch/quantization/utils.py` → `fuse_spconv_bn_eval` | 產生 `SparseConvolution.bias` | `bias_cur` 為 `None`，export 多為 **5-input** |
...
| **⑥ sparse INT8 INT8 transform** | `deployment/projects/bevfusion_l/export/sparse_int8_onnx_transform.py` | FP `ImplicitGemm` 若有 **6 input**，將第 6 路常數 **併入 `bias_scaled`**（`bs_arr + ex / output_scale`） | INT8 與 FP16 fuse 後 **靜默錯精度** |
| **⑦ INT8 TRT plugin** | `deployment/projects/bevfusion_l/cpp/int8_plugin/implicit_gemm_int8_plugin.hpp` | 固定 **7 input**；index **6** = `bias_scaled`（≠ FP 第 6 路 tensor 名，語意為量化後 bias） | INT8 engine 缺 bias 項 |
...
```1032:1056:deployment/projects/bevfusion_l/export/sparse_int8_onnx_transform.py
        if len(node.input) == 6:
...
`ImplicitGemmInt8` plugin 契約見 `deployment/projects/bevfusion_l/cpp/int8_plugin/implicit_gemm_int8_plugin.hpp`（`IN_BIAS_SCALED = 6`）。
