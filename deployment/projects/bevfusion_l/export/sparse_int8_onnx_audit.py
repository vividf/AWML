"""Read-only audit of sparse INT8 ``ImplicitGemmInt8`` ONNX (plugin scales + channel tensors).
...
Example::
    ...
    python -m deployment.projects.bevfusion_l.export.sparse_int8_onnx_audit \\
...
def audit_implicit_gemm_int8_onnx(model: onnx.ModelProto, *, stream=sys.stdout) -> int:
    ...
    if count == 0:
        print(
            "No INT8 autoware::ImplicitGemm (precision=1) nodes found (expect sparse INT8 INT8 ONNX).",
            file=stream,
        )
    else:
