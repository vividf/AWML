...
Debugging / scale audit::
    ...
    python -m deployment.projects.bevfusion_l.export.sparse_int8_onnx_transform ... --audit-report int8_layers.json

    # Read-only dump of an already-transformed INT8 ONNX (no checkpoint)
    python -m deployment.projects.bevfusion_l.export.sparse_int8_onnx_audit --onnx sparse_int8.onnx
"""
...
from deployment.projects.bevfusion_l.export.onnx_fuse_implicit_gemm_activation import _try_get_constant_numpy
from deployment.projects.bevfusion_l.export.sparse_int8_transform_ops import (
    _LEGACY_W_AMAX_PTQ,
...
def build_sparse_int8_post_transform(
    *,
    checkpoint_path: str,
    fp16_layer_patterns: Optional[List[str]] = None,
    verbose: bool = False,
    ...
    """Build an ``OnnxExportPipeline`` post-transform that rewrites the sparse ONNX to ImplicitGemmInt8.

    Reads the PTQ checkpoint's calibrated ``_amax`` and returns a ``proto -> proto`` transform
    for the sparse component's ``post_transforms``. This is the new-framework replacement for the
    old fork's file-based ``_postprocess_sparse_onnx_int8``: the shared pipeline owns ONNX
    load/save, so no separate FP16 backup file is written (the transform is deterministic and
    re-runnable from the exported FP graph + the checkpoint).

    Args:
        checkpoint_path: PTQ checkpoint (``.pth``) carrying sparse ``_amax`` entries.
        fp16_layer_patterns: substring patterns of ImplicitGemm node names to keep FP16
            (``spconv_int8_fp16_layers``).
        verbose: forward to :func:`transform_onnx_int8` for per-node logging.
        fuse_implicit_gemm_trailing_relu: fold trailing ``Relu`` into ImplicitGemm ``act_type``.
    """
    patterns = [str(p) for p in (fp16_layer_patterns or []) if str(p).strip()]

    def _transform(model_proto: onnx.ModelProto) -> onnx.ModelProto:
        if not checkpoint_path:
            raise RuntimeError(
                "INT8 sparse ONNX transform requires a checkpoint_path (PTQ checkpoint with _amax entries)."
            )
        amax_dict = _load_amax_from_checkpoint(checkpoint_path)
        if not amax_dict:
            raise RuntimeError(
                f"PTQ checkpoint {checkpoint_path!r} has no _amax keys; cannot transform sparse ONNX to INT8."
            )
        ...
        return transform_onnx_int8(
            model_proto,
            layer_scales,
            encoder_sd,
            verbose=verbose,
            amax_dict=amax_dict,
            fp16_layer_patterns=patterns,
            fuse_implicit_gemm_trailing_relu=fuse_implicit_gemm_trailing_relu,
        )
