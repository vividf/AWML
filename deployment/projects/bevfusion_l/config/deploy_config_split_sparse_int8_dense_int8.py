...
**CLI**（需在含 pytorch-quantization 的環境；Docker 內用 ``pip install --no-cache-dir --index-url https://pypi.nvidia.com --extra-index-url https://pypi.org/simple pytorch-quantization==2.1.3``，詳見 ``deploy_config_int8.py`` 註解）::
    ...
    python -m deployment.cli.main bevfusion \\
        deployment/projects/bevfusion_l/config/deploy_config_split_int8_all.py \\
...
#       --deploy-cfg deployment/projects/bevfusion_l/config/deploy_config_split_int8_all.py \\
#       --sparse-int8-only \\
#       --calibrate-samples 256 --batch-size 1 --calib-seed 0 \\
...
# checkpoint_path = "vivid/bench_comparison/bevfusion_2_7/best_epoch_28_ptq_sparse_int8_dense_int8.pth"  # personal scratch path — not reproducible
