...
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"          # deployment/projects/bevfusion_l/experimental
...
DEPLOY_CFG="${DEPLOY_CFG:-deployment/projects/bevfusion_l/config/deploy_config_split_int8.py}"
...
cd "${WORKSPACE}"
python -m deployment.projects.bevfusion_l.experimental.export_sparse_encoder_int8 \
    --config "${CONFIG}" \
