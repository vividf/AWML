...
與 **AWML TensorRT Path B** 的對照：**兩邊都是在 plugin 或本 op 內把特徵／權重變成 int8，再進同一 family 的 `implicit_gemm`**；差別在於 **誰解析 ONNX**（這裡是 **自訂 parser + Engine**，AWML 是 **TRT + 自訂 plugin**）。
...
### 8.1 權重量化時機：開源做一次，Path B 目前每 frame 做一次

**New3D**：在引擎 **`build()` → `SparseConvolution::configure()`** 階段，把 **FP16 權重**量成 **int8**，並算好 **`scale_`／bias 的 FP32 融合**（見 §5.3）。之後 **`forward` 只處理會變的特徵與索引**。

**AWML Path B**（`deployment/projects/bevfusion/cpp/int8_plugin/implicit_gemm_int8_plugin.cpp` 的 **`enqueue`**）在 **每次推理** roughly 會做：
...
若目前僅使用 **FP16 ImplicitGemm plugin** 而 **GPU 世代支援** int8 tensor core 路徑，啟用 **Path B（ImplicitGemmInt8）** 往往是稀疏塔 **吞吐／延遲** 最直觀的升級；**精度**需以 PTQ／校準集驗證。
...
| 高 | **啟用 Path B INT8**（若尚未啟用且 GPU 適合） | 與開源同底層 int8 GEMM。 |
