...
| **1** | PTQ，產含 `_amax` 的 `.pth` | `python -m deployment.projects.bevfusion.quantization.quantize ptq ... --deploy-cfg deployment/projects/bevfusion/config/deploy_config_split_int8.py ...`；若只要稀疏塔校準、與 **Preset C** 一致，建議加 **`--sparse-int8-only`**（見 deploy 檔頭註解） |
...
| 路徑 | 角色 |
|------|------|
| `deployment/projects/bevfusion/experimental/libspconv_onnx_exporter.py` | 自訂 SparseConvolution ONNX 導出方向 |
| `deployment/projects/bevfusion/experimental/cpp/libspconv_trt_bridge.{hpp,cpp}` | 載入 sparse ONNX / engine + dense TRT，配置見檔內 `BridgeConfig` |
