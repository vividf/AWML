...
相關延伸：`README_PTQ_INT8_SPCONV_DEPLOYMENT.md`（端到端管線）、`README_NEW3D_LIDAR_OPEN_SPCONV.md`（Lidar 開源 New3D：`spconv::Engine`、自訂 ONNX、INT8 鏈路；**§8** 為 AWML 推理加速與開源作法對照）、`cpp/int8_plugin/README.md`（`output_scale`／epilogue）、`11_int8_pathb_autoware_plugin.md`。
...
| **TensorRT + Path B `ImplicitGemmInt8` + `libimplicit_gemm_int8_plugin.so`** | **有**：在 plugin 的 `enqueue` 內把特徵與權重變成 **`tv::Tensor` `int8`**，呼叫 **`ConvGemmOps::implicit_gemm`**，tuner 選 **`is_int8_inference`** 的演算法（與 spconv Python 走 C++ 路徑時相同底層）。 |

所以：**「AWML 專案有沒有用 INT8」取決於你指 PyTorch 還是 TRT、有沒有開 Path B。** PTQ checkpoint 裡的 `_amax` 是 **刻度**；**真正把乘加放在 INT8 上的是 Path B TRT plugin（以及 spconv 官方的量化推理路徑）。**
...
- `spconv/docs/TENSORRT_INT8_GUIDE.md`：**scale／bias 公式**、在 **custom plugin** 裡呼叫 `ConvGemmOps::implicit_gemm`（與 AWML Path B 同一 C++ API）。
...
### 4.3 官方 scale 公式（與 AWML Path B 一致）
...
| **TRT 稀疏卷積** | 文件建議 **plugin 內**呼叫 `implicit_gemm` | **Path B**：`ImplicitGemmInt8` plugin → **`tv::int8` + `implicit_gemm`** |
