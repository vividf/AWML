...
>
> 配合閱讀：
> - 設計與里程碑（含 profile 數據、export 手術細節）：[`BEVFusion_spconv_DDS_optimization.md`](BEVFusion_spconv_DDS_optimization.md)
...
- 板上 profile（見 [`BEVFusion_spconv_DDS_optimization.md`](BEVFusion_spconv_DDS_optimization.md) §2）：單次推論 34.5ms，其中 **GPU idle ≈ 10.3ms ≈ 29.7%**；6 個 trainStation 對應 4 個下採樣的 `DeviceToShapeHostCopy`。
...
`export/sparse_trainstation_transform.py::remove_trainstation_dds`：
...
3. **量測結果**（見 [`BEVFusion_spconv_DDS_optimization.md`](BEVFusion_spconv_DDS_optimization.md) §8）：
...
| 角色 | 檔案 |
|------|------|
| Export 圖手術 | `deployment/projects/bevfusion/export/sparse_trainstation_transform.py`（`remove_trainstation_dds`、`rulebook_input_name`） |
| Export pipeline 串接 / merge | `export/onnx_export_pipeline.py`（`_postprocess_sparse_onnx_fp`、`_merge_split_onnx`） |
| Deploy 設定（含 16 input 的 TRT profile） | `config/deploy_config_split_fp16_opt_trainstation.py`（`spconv_remove_trainstation=True`） |
| AWML eval runtime（Python 參考實作） | `pipelines/sparse_rulebook_precompute.py`、`pipelines/tensorrt.py` |
