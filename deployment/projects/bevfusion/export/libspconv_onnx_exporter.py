...
class LibspconvExporter:
    ...
    def _trace_sparse_conv(self, module: nn.Module, x: Any, y: Any) -> None:
        ...
        print(f"   --> SparseConvolution{idx}[{label}]  "
              f"Input {self._get_id(x)}, Output {self._get_id(y)}")
    ...
    def _trace_dense(self, sct: Any, y: torch.Tensor) -> None:
        ...
        print(f"   --> ToDense{idx}[{sct.spatial_shape}][{list(y.size())}]  "
    ...
    def _trace_permute(self, x: torch.Tensor, y: torch.Tensor, dims: tuple) -> None:
        ...
        print(f"   --> Permute{idx}[{dims}][{list(y.shape)}]  "
              f"Input {self._get_id(x)}, Output {self._get_id(y)}")
        self._nodes.append(
            helper.make_node(
                "Transpose",
...
def extract_dynamic_ranges_from_checkpoint(
    ...
    for name, module in model.named_modules():
        ...
        if idr is not None or wdr is not None:
            count += 1
            print(f"  [dynamic-range] {name}: input_dr={idr}, "
                  f"weight_dr_len={len(wdr) if wdr else 0}")
