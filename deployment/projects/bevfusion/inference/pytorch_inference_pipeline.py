...
class BEVFusionPyTorchPipeline(BEVFusionDeploymentPipeline):
    ...
    def run_bevfusion(
        ...
        # Not used: this pipeline overrides run_model() to call _run_bevfusion_with_breakdown()
        # directly, so the base template never dispatches through run_bevfusion(). Present only to
        # satisfy the abstract method on BEVFusionDeploymentPipeline.
        raise NotImplementedError("BEVFusionPyTorchPipeline overrides run_model(); run_bevfusion() is not used.")
