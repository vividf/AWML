...
def build_bevfusion_model(
    ...
    *,
    ...
    if quantization and quantization.get("enabled", False):
        try:
            ...
            if quantization.get("ptq_checkpoint"):
                raise RuntimeError(
                    ...
                    "Typical causes: (1) deploy PTQ load order must match bevfusion/quantization/quantize.py "
        ...
        load_checkpoint(model, checkpoint_path, map_location=torch_device)
        if fuse_spconv_bn:
            # FP32 path: fuse sparse BN only (no quantizers), via the sparse scheme's fuse mode.
            from deployment.projects.bevfusion.quantization.schemes import SpconvInt8Scheme

            SpconvInt8Scheme(int8=False, fuse_bn=True).prepare(model)
...
def _load_with_quantization(
    ...
    """
    from deployment.projects.bevfusion.quantization.plan import build_bevfusion_plan
        ...
        if spconv_int8:
            ...
            if miss_sparse_bn and unexp_legacy_obs:
                logger.error(
                    ...
                    "bevfusion/quantization/quantize.py (sparse _amax keys)."
                )
