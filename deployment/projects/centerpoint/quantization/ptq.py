...
def quantize_ptq(
    ...
    3. Attach residual-add quantizers if quant_add=True (for ResNet-style backbones)
    ...
    Args:
        ...
        quant_add: Whether to attach residual-add quantizers to residual blocks (BasicBlock/SparseBasicBlock).
    ...
    Example:
        >>> from deployment.quantization import quantize_ptq
    ...
    if quant_add and verbose:
        print("  - residual-add quantizers attached to residual blocks (BasicBlock/SparseBasicBlock)")
    # Step 3: Calibrate
    if verbose:
...
def load_ptq_model(
    ...
    2. Attaches residual-add quantizers if quant_add=True (to match saved model structure)
    ...
    Args:
        ...
        quant_add: Whether residual-add quantizers were attached during PTQ (must match saved model)
