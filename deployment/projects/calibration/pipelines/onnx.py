...
class CalibrationONNXPipeline(CalibrationDeploymentPipeline):
    ...
    def __init__(
        ...
        Args:
            onnx_path: Path to ONNX model file or directory containing it
        ...
        # Resolve ONNX path using components config
        ...
        self.session = ort.InferenceSession(resolved_path, sess_options, providers=providers)
        ...
        super().__init__(
            model=self.session, device=device, num_classes=num_classes, class_names=class_names, backend_type="onnx"
        )

        logger.info(f"Initialized CalibrationONNXPipeline on {device}")
        logger.info(f"  ONNX model: {resolved_path}")
