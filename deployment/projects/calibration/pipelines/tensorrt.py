...
class CalibrationTensorRTPipeline(GPUResourceMixin, CalibrationDeploymentPipeline):
    ...
    def __init__(
        ...
        Args:
            engine_path: Path to TensorRT engine file or directory containing it
            ...
            components_cfg: Component configuration from deploy_config for resolving file paths
        """
        try:
        ...
        # Resolve engine path using components config
        resolved_path = resolve_artifact_path(
            base_dir=engine_path,
        ...
        if self.engine is None:
            raise RuntimeError(f"Failed to load TensorRT engine from {resolved_path}")
        ...
        logger.info(f"Loaded TensorRT engine from: {resolved_path}")
        ...
        super().__init__(
            model=self.engine, device=device, num_classes=num_classes, class_names=class_names, backend_type="tensorrt"
        )

        logger.info(f"Initialized CalibrationTensorRTPipeline on {device}")
        logger.info(f"  Engine: {resolved_path}")
