"""ONNX tracing module for the ``pts_head_memory`` component (decoder head + memory queue).

Verbatim port of ``TrtPtsHeadContainer`` in ``projects/StreamPETR/deploy/containers.py``.

The temporal memory queue is explicit graph I/O: ``pre_memory_*`` tensors come in
(``[1, memory_len, …]``), the head prepends this frame's top-k proposals, and the grown
``post_memory_*`` tensors (``[1, memory_len + topk, …]``) go out — the host runtime slices
them back to ``memory_len`` before the next frame. Keeping the queue outside the graph keeps
the engine stateless.

``data_timestamp`` is traced but unused (the in-graph timestamp arithmetic is intentionally
done outside TensorRT — see the commented lines, kept verbatim); onnxsim prunes the input.
"""
...
class StreamPETRPtsHeadONNX(torch.nn.Module):
    """Traces the StreamPETR decoder head with explicit memory-queue I/O.

    The head forward is re-implemented here (rather than calling
    ``StreamPETRHead.forward``) because the training forward interleaves memory updates with
    non-traceable bookkeeping; this module is the deployed-graph contract. Denoising must be
    disabled before tracing (``head.with_dn = False`` — the component builder owns that).
    """
