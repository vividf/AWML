...
class T4MetricV2(BaseMetric):
    ...
    def _parse_ground_truth_from_sample(self, time: float, data_sample: Dict[str, Any]) -> FrameGroundTruth:
        ...
        if self.min_num_points > 0 and len(bboxes):
            ...
            assert len(num_lidar_pts) == len(bboxes), (
                f"min_num_points={self.min_num_points} needs a num_lidar_pts entry per GT box, got "
                f"{len(num_lidar_pts)} for {len(bboxes)} boxes; the pipeline must carry num_lidar_pts "
                f"into eval_ann_info."
            )
