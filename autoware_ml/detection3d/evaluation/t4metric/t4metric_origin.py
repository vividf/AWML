...
def lidar_nusc_box_to_global(
    ...
    box_list = []
    #target_global_xy = np.array([65399.11467,  640.89627]) 
    #tolerance = 0.0005
    for box in boxes:
        # ➕ Backup 原始位置（lidar/ego frame）
        original_center = box.center.copy()
        ...
        # print("Ego center: ", box.center)

        # Step 2: filter by detection range (still in ego frame)
        ...
        # print("Ego to Global Transformation Matrix:\n", ego2global)

        box.rotate(pyquaternion.Quaternion(matrix=ego2global, rtol=1e-05, atol=1e-07))
        box.translate(ego2global[:3, 3])
        # print("Global center: ", box.center)



        # # ✅ Debug: check if global coord matches target
        # dist = np.linalg.norm(box.center[:2] - target_global_xy)
        # if dist < tolerance:
        #     print("[🔍 DEBUG MATCH FOUND]")
        #     print(f"Original (lidar/ego) center: {(original_center[:3])}")
        #     print(f"Transformed (global) center: {(box.center[:3])}")
        #     print(f"Distance to target: {dist:.4f}")

        #     # create nuscene boxes
        #     test_gt_box = box.copy()
        #     test_gt_box.center = [-4.7819214, -35.89899, 0.25421768]

        #     lidar2ego = np.array(info["lidar_points"]["lidar2ego"])
        #     test_gt_box.rotate(pyquaternion.Quaternion(matrix=lidar2ego, rtol=1e-05, atol=1e-07))
        #     test_gt_box.translate(lidar2ego[:3, 3])

        #     # Step 2: filter by detection range (still in ego frame)
        #     cls_range_map = eval_configs.class_range
        #     radius = np.linalg.norm(test_gt_box.center[:2], 2)
        #     det_range = cls_range_map[classes[test_gt_box.label]]
        #     if radius > det_range:
        #         continue

        #     # Step 3: ego → global
        #     ego2global = np.array(info["ego2global"])
        #     test_gt_box.rotate(pyquaternion.Quaternion(matrix=ego2global, rtol=1e-05, atol=1e-07))
        #     test_gt_box.translate(ego2global[:3, 3])


        #     print("GT after transformation box: ", test_gt_box.center[:3])
