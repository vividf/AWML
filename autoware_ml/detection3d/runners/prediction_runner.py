...
from autoware_ml.detection3d.visualization.decoded_bboxes import DecodedBboxes, BatchDecodedBboxes


def visualization_wrapper(batch_decoded_bboxes: BatchDecodedBboxes,
                          vis_dir: Path, xlim: Tuple[int, int],
...
class PredictionRunner(BaseRunner):
    ...
    def run(self) -> None:
        ...
        batch_decoded_bboxes = self.predict(
    ...
    def _visualize(self,
                   batch_decoded_bboxes: List[BatchDecodedBboxes]) -> None:
        ...
        with concurrent.futures.ProcessPoolExecutor(
            ...
            with tqdm(
                    total=len(batch_decoded_bboxes),
                    desc="Visualizing") as pbar:
                # Submit tasks to the executor and store futures
                futures = [
                    executor.submit(partial_visualization_wrapper,
                                    decoded_bboxes)
                ...
                for future in concurrent.futures.as_completed(futures):
                    _ = future.result()
                    pbar.update(
                        1)  # Update progress bar for each completed task
    ...
    def _decode_outputs(self, output: Det3DDataSample,
                        ...
                        data_sample: Det3DDataSample) -> BatchDecodedBboxes:
        """
    ...
    def predict(self, model: nn.Module,
                dataloader: DataLoader) -> List[BatchDecodedBboxes]:
        """
        ...
        with torch.no_grad():
            ...
            for index, data in enumerate(
                ...
                if self._batch_size > 0 and len(
                        batch_decoded_bboxes) % self._batch_size == 0:
                    self._visualize(batch_decoded_bboxes=batch_decoded_bboxes)
                    batch_decoded_bboxes = []
