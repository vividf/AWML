...
def create_info_data(
    ...
    non_annotated_info_file_path: Path = _create_non_annotated_info(
        cfg=model_config, dataset_version=Path(non_annotated_dataset_path).stem, logger=logger
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create pseudo labeled info data from non-annotated dataset",
    )
