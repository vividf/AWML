...
class PseudoLabelGenerator3D:
    def __init__(
        ...
        self.dataset_annotations: Dict[str, SceneAnnotations] = {
            scene_id: SceneAnnotations(scene_id=scene_id, scene_annotations_dict={}) for scene_id in scene_ids
        }
    ...
    def dump(self, overwrite: bool) -> None:
        ...
        for scene_id, dataset_annotation in self.dataset_annotations.items():
            ...
            input_dir = Path(
                get_scene_root_dir_path(
                    self.non_annotated_dataset_path.parent,
                    self.non_annotated_dataset_path.name,
                    scene_id,
                )
            )
