...
def parse_args():
    ...
    parser.add_argument("checkpoint_dir", help="the directory path that contain checkpoints")
    parser.add_argument("min_epoch", help="the epoch you want to start to test")
...
def main():
    ...
    # get checkpoint list
    file_list = os.listdir(args.checkpoint_dir)
    checkpoint_list = []
    for file_name in file_list:
        if "pth" in file_name:
            file_numbers = re.findall(r"\d+", file_name)
            epoch_number = int(file_numbers[0])
            if epoch_number >= int(args.min_epoch):
                checkpoint_list.append(file_name)
    checkpoint_list.sort()

    for checkpoint_file in checkpoint_list:
        checkpoint = osp.join(args.checkpoint_dir, checkpoint_file)
