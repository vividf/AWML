...
def parse_args():
    parser = argparse.ArgumentParser(description="Export CenterPoint model to backends.")
    ...
    parser.add_argument(
        "--data-root", default="", help="the dir to save datasets. Set it to overwrite the default config"
    )
    parser.add_argument(
        "--bboxes-score-threshold", default=0.10, type=float, help="Score threshold to filter out bboxes"
    )
    parser.add_argument("--frame-range", default=None, nargs="*", type=int, help="Index to render data")
    parser.add_argument(
        "--ann-file-path", default="", help="the dir to save pkl infos. Set it to overwrite the default config"
    )
    parser.add_argument("--log-level", help="set log level", default="INFO", choices=list(logging._nameToLevel.keys()))
    parser.add_argument("--device", choices=["cpu", "gpu"], default="gpu", help="Set running device!")
    parser.add_argument("--max_workers", type=int, default=8, help="Maximum cpu workers running in multiprocessing")
...
def build_prediction_runner(args) -> PredictionRunner:
    ...
    work_dir = (
        Path(os.getcwd()) / "work_dirs" / "predictions" / experiment_name if not args.work_dir else Path(args.work_dir)
    )
