...
class Waymo2KITTI(object):
    ...
    def convert(self):
        ...
        if self.workers == 0:
            ...
            data_infos = mmengine.track_parallel_progress(self.convert_one, range(len(self)), self.workers)
        data_list = []
        ...
        filenames = osp.join(osp.dirname(self.save_dir), f"{self.info_prefix + self.info_map[self.split]}")
    ...
    def convert_range_image_to_point_cloud(
        ...
        for c in calibrations:
            ...
            if c.name == 1:
                mask_index = (ri_index * range_image_mask.shape[0] + mask_index[:, 0]) * range_image_mask.shape[
                    1
                ] + mask_index[:, 1]
    ...
    def create_waymo_info_file(self, frame, file_idx, frame_idx, file_infos):
        ...
        frame_infos["ego2global"] = np.array(frame.pose.transform).reshape(4, 4).astype(np.float32).tolist()
        ...
        for i, (cam_key, camera_calib, Tr_velo_to_cam) in enumerate(
            ...
            cam_infos["lidar2img"] = (camera_calib @ Tr_velo_to_cam).astype(np.float32).tolist()
        ...
        if not self.test_mode:
            ...
            if self.save_cam_sync_instances:
                frame_infos["cam_sync_instances"] = self.gather_instance_info(frame, cam_sync=True)
    ...
    def gather_instance_info(self, frame, cam_sync=False):
        ...
        for obj in frame.laser_labels:
            ...
            instance_info["bbox_3d"] = (
                np.array([x, y, z, length, width, height, rotation_y]).astype(np.float32).tolist()
            )
        ...
        for cam_type in self.camera_types:
            ...
            for instance in instances:
                ...
                final_coords = post_process_coords(
                    corner_coords, imsize=(images["CAM_FRONT"]["width"], images["CAM_FRONT"]["height"])
                )
                ...
                cam_instance["bbox_3d"] = gt_bboxes_3d.numpy().squeeze().astype(np.float32).tolist()
                cam_instance["bbox_label_3d"] = instance["bbox_label_3d"]

                center_3d = gt_bboxes_3d.gravity_center.numpy()
                center_2d_with_depth = points_cam2img(center_3d, cam2img, with_depth=True)
    ...
    def merge_trainval_infos(self):
        ...
        train_infos_path = osp.join(osp.dirname(self.save_dir), f"{self.info_prefix}_infos_train.pkl")
        val_infos_path = osp.join(osp.dirname(self.save_dir), f"{self.info_prefix}_infos_val.pkl")
        train_infos = mmengine.load(train_infos_path)
        val_infos = mmengine.load(val_infos_path)
        trainval_infos = dict(
            metainfo=train_infos["metainfo"], data_list=train_infos["data_list"] + val_infos["data_list"]
        )
        mmengine.dump(trainval_infos, osp.join(osp.dirname(self.save_dir), f"{self.info_prefix}_infos_trainval.pkl"))
