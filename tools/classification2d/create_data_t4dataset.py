...
def update_detection_data_annotations(data_list: Dict[str, DataEntry],
    ...
    for annot_dict in object_ann:
        category = categories[annot_dict["category_token"]]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config", type=str, required=True, help="config for T4dataset")
    parser.add_argument(
        "--root_path",
        type=str,
        required=True,
...
def get_detection_data_empty_dict(data_name: str,
                                  classes: List[str]) -> DetectionData:
...
def assign_ids_and_save_detection_data(split_name: str,
                                       data_entries: List[DataEntry],
                                       out_dir: str, data_name: str,
...
def main() -> None:
        ...
        for split in ["train", "val", "test"]:
            ...
            for scene_id in dataset_list_dict.get(split, []):
                ...
                scene_root_dir_path = get_scene_root_dir_path(
                    args.root_path, dataset_version, scene_id)
                ...
                for tmp in nusc.sample_data:
                    data_entry = DataEntry(
                        img_path=os.path.abspath(
                            os.path.join(nusc.dataroot, tmp["filename"])),
                        width=tmp["width"],
                        height=tmp["height"],
                    )
    ...
    for split in ["train", "val", "test"]:
        assign_ids_and_save_detection_data(split, data_infos[split],
                                           args.out_dir, args.data_name,
                                           cfg.classes)

    # Save combined splits
    assign_ids_and_save_detection_data("trainval",
                                       data_infos["train"] + data_infos["val"],
                                       args.out_dir, args.data_name,
                                       cfg.classes)
    assign_ids_and_save_detection_data(
        "all", data_infos["train"] + data_infos["val"] + data_infos["test"],
        args.out_dir, args.data_name, cfg.classes)
