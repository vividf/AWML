...
def parse_args():
    parser = argparse.ArgumentParser(description="MMPreTrain test (and eval) a model")
    ...
    parser.add_argument(
        ...
        help="To output whether metrics or predictions. " "Defaults to output predictions.",
    )
    parser.add_argument(
        "--cfg-options",
        nargs="+",
    ...
    parser.add_argument("--amp", action="store_true", help="enable automatic-mixed-precision test")
    parser.add_argument("--show-dir", help="directory where the visualization images will be saved.")
    parser.add_argument("--show", action="store_true", help="whether to display the prediction results in a window.")
    ...
    parser.add_argument(
        "--no-pin-memory", action="store_true", help="whether to disable the pin_memory option in dataloaders."
    )
    parser.add_argument(
        "--tta",
        action="store_true",
...
def main():
    ...
    if args.out is None and args.out_item is not None:
        raise ValueError(
            "Please use `--out` argument to specify the " "path of the output file before using `--out-item`."
        )
