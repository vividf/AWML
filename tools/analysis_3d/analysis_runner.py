...
class AnalysisRunner:
    ...
    def _extract_sample_data(self, nusc: NuScenes) -> Dict[str, SampleData]:
        ...
        for sample in nusc.sample:
            # Extract sample data
            nuscene_sample_data = extract_nuscene_sample_data(
                sample=sample, nusc=nusc)

            # Get box attrs
            box_attrs = get_box_attrs(
                boxes=nuscene_sample_data.boxes, nusc=nusc)

            # Convert to SampleData
            sample_data[sample["token"]] = SampleData.create_sample_data(
                sample_token=sample["token"],
                boxes=nuscene_sample_data.boxes,
                box_attrs=box_attrs)
        return sample_data
    ...
    def _extra_scenario_data(
        ...
        for scene_token in scene_tokens:
            ...
            nusc = NuScenes(
                version="annotation",
                dataroot=str(scene_root_dir_path),
                verbose=False)
