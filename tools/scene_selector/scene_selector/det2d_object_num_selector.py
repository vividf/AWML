...
class Det2dObjectNumSelector(ImageBasedSceneSelector):
    ...
    def is_target_scene(self, image_array: Union[List[np.ndarray],List[str]], return_counts: bool = False, results_path: str = "") -> Union[bool, tuple]:
        """
    ...
    def _count_labels(self, results: Dict) -> Dict[str, int]:
        ...
        for image_result in results.get("predictions", []):
            valid_labels = [
                label for label, score in zip(image_result["labels"], image_result["scores"])
