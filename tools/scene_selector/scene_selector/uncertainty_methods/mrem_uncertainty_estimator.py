...
from mmdet3d.structures import Det3DDataSample, ops as box_np_ops, bbox_overlaps_3d
...
class REMCustomInferencer(LidarDet3DInferencer):
    ...
    def _init_pipeline(self, cfg: ConfigType) -> Compose:
        ...
        pipeline_cfg[load_point_idx][
            'type'] = 'LidarWithCamDet3DInferencerLoader'
...
class ModelRareExampleMining:
    ...
    def __call__(self, sensor_info, results_path: str = ""):
        ...
        for i, all_model_preds in enumerate(zip(*preds)):
            ...
            unique_bboxes_inst, data_inst, variances_inst, h_i_inst, r_i_inst = self.calculate_scores(all_model_preds, pcd)
    ...
    def calculate_scores(self, model_predictions, point_cloud):
        ...
        for c in range(n_components):
            ...
            scores_per_model = [
                np.max(all_scores[indices[cluster_model_indices == model_idx]]) if model_idx in cluster_model_indices else 0
    ...
    def plot_bev(self, pcd, model_bboxes, model_scores, model_names, unique_bboxes_inst, output_path):
        ...
        mask = (pcd[:, 0] >= -self.d_thresh) & (pcd[:, 0] <= self.d_thresh) & (pcd[:, 1] >= -self.d_thresh) & (pcd[:, 1] <= self.d_thresh)
    ...
    def _plot_bbox_and_score(self, bbox, score, color, linestyle=None):
        ...
        plt.text(x, y, f"{score:.2f}", color=color, alpha=0.75, fontsize=10, ha='center', va='center', fontweight='bold')
